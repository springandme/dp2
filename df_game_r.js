...
// 获取城镇id
var CUser_get_vill = new NativeFunction(ptr(0x8645564), 'int', ['pointer'], {"abi": "sysv"});
// 获取城镇中的小区域
var CUser_get_area = new NativeFunction(ptr(0x86813BE), 'int', ['pointer'], {"abi": "sysv"});
//获取站街坐标 - X轴
var CUser_get_posX = new NativeFunction(ptr(0x0813492C), 'int', ['pointer'], {"abi": "sysv"});
//获取站街坐标 - Y轴
var CUser_get_posY = new NativeFunction(ptr(0x0813493C), 'int', ['pointer'], {"abi": "sysv"});
...
//获得QP (user, qp_cnt, qst_id, log_bool) CUser_incQuestPoint(user, 677, 0, 0)
...
function CMailBoxHelperReqDBSendNewSystemMail(User, item_id, item_count, mail_title, mail_contact) {
    var ret_item = find_item(item_id);
    if (ret_item) {
        var Inven_ItemPr = Memory.alloc(100);
        Inven_Item(Inven_ItemPr); // 清空道具
        var itemid = GetItem_index(ret_item);
        var item_type = ret_item.add(8).readU8();
        Inven_ItemPr.writeU8(item_type);
...
function api_CUser_AddItem(user, item_id, item_cnt) {
    ...
    if (slot >= 0) {
        // 通知客户端有游戏道具更新
        CUser_SendUpdateEqu_JewelSocket(user, item_space.readInt(), slot);
    }
...
function init_db() {
    ...
    // 镶嵌建表 - 创建数据库，排序从150开始，也可以从大一点的数值开始
...
function on_end_event_villageattack() {
    ...
    if (villageAttackEventInfo.defend_success) {
        ...
        api_gameworld_foreach(function (user, args) {
            ...
            if (Inven_Item_getKey(equ)) {
                ...
                if (upgrade_level < 31) {
                    ...
                    // 通知客户端更新装备
                    CUser_SendUpdateEqu_JewelSocket(user, 3, slot);
                }
            }
        }, null);
        // 榜一大哥
        ...
        api_gameworld_foreach(function (user, args) {
            ...
            // 通知客户端更新金币数量
            CUser_SendUpdateEqu_JewelSocket(user, 0, 0);
        }, null);
        // 频道内公告活动已结束
...
function fix_TOD(skip_user_apc) {
    ...
    // 修复金币异常
    // CParty::BuyCeraShopLimitItem  购买限时商品 0x821F9BA Cb_BuyCeraShopLimitItem_Enter_Func <- {}; Cb_BuyCeraShopLimitItem_Leave_Func <- {};
    // CParty::UseAncientDungeonItems  使用远古地下城道具 0x859EAC2
...
function fix_use_emblem() {
    ...
    Interceptor.replace(ptr(0x0821A412), new NativeCallback(function (Dispatcher_AddSocketToAvatar, CUser, PacketBuf) {// 装备开孔
        ...
        try {
            ...
            CUser_SendUpdateEqu_JewelSocket(CUser, 0, equ_slot);
            var packet_guard = api_PacketGuard_PacketGuard();
    ...
    Interceptor.attach(ptr(0x8217BD6), {
        onEnter: function (args) {
            try {
                ...
                // log('收到角色[' + api_CUserCharacInfo_getCurCharacName(user) + ']的镶嵌请求');
...
function auto_unseal_random_option_equipment() {
    // CInventory::insertItemIntoInventory  玩家新增道具时 0x8502D86
    // CInventory::getItemIntoInventory  玩家捡起道具 0x85B949C
...
function enable_drop_use_luck_point() {
    ...
    // CLuckPoint::GetItemRarity  掉落道具 0x8550BE4
...
function epicPotion() {
    ...
    Interceptor.replace(ptr(0x8550BE4), new NativeCallback(function (a1, a2, roll, a4) {
        ...
        // debug 只生效在单人深渊模式，不强求为 用来 史诗药剂
        // if (this.returnAddress == 0x853583a && CUser_CheckCoolTimeItem(user, global_config["item_handle"]["epicPotion"]) && api_GetPartyMemberNumber(Party) == 1) {
        if (this.returnAddress == 0x853583a && api_GetPartyMemberNumber(Party) == 1) {
            var MaxRoll = a2.add(16).readU32();
            var odds = global_config["user"]["abyss_odds"];// 默认药剂的增加几率
            var charac_no = CUserCharacInfo_getCurCharacNo(user);
            if (global_config["vip_user"]['' + charac_no + '']) {// 特殊VIP角色的几率
                odds = global_config["vip_user"]['' + charac_no + '']["abyss_odds"];
            }
...
function DP_Strengthen_SendUpdateItemList() {
    Interceptor.attach(ptr(0x080FC850), {
        ...
        onLeave: function (retval) {
            CUser_SendUpdateEqu_JewelSocket(this.user, 0, this.equiPos);
        }
    });
}
...
// 深渊次次困难
function setHellPartyDifficulty() {
    Interceptor.attach(ptr(0x082FFA2E), {
        onLeave: function (retval) {
            retval.replace(1);
        }
    });
    // int __cdecl = CBattle_Field::GetHellPartyDifficulty(CParty **a1, int a2)
    Interceptor.attach(ptr(0x082FFA2E), {
        onLeave: function (retval) {
            retval.replace(1);
        }
...
function dispatcherModItemAttr() {
    Interceptor.replace(ptr(0X81DD2BE), new NativeCallback(function (a1, user, packet) {
            // DispatcherResetRandomOption::process
            var inventory = CUserCharacInfo_getCurCharacInvenW(user);
            var stk_slot = packet.add(16).readU16();        // 获取调整箱槽位
            var inven_stk = CInventory_GetInvenRef(inventory, 1, stk_slot);   // 获取调整箱
            var stk_id = inven_stk.add(2).readU32();        // 获取调整箱ID
            var equ_slot = packet.add(14).readU16();        // 获取装备槽位
            var inven_equ = CInventory_GetInvenRef(inventory, 1, equ_slot);   // 获取装备
            var equ_id = inven_equ.add(2).readU32();        // 获取装备ID
            var ran_s = packet.add(18).readU8();            // 获取玩家选的第几个词条 0=37 1=40 2=43 47无法选择
            var ran_no = 37 + ran_s * 3;                      // 计算出实际的词条槽位

            console.log('[' + get_timestamp() + '] [frida] [info] --------------------------- dispatcherModItemAttr: stk_id: ' + stk_id + '----------------------------');
            // 假紫词条数值在原版PVF中多个 大概是紫装S=50 粉装S=100 根据自己情况写 最大不可超过255
            if (stk_id == 15 || stk_id == 690000114) { // 若调整箱ID是897 将调整的这个词条改成S级  // 装备品级调整箱 - 690000114 | 15
                inven_equ.add(ran_no + 1).writeU8(50); // 修改假紫词条数值1=50
            ...
            if (stk_id == 897) {           // -- 黄金品级调整箱 897
                // 若调整箱ID是 897 将调整的所有词条改成S级
                inven_equ.add(38).writeU8(50);
                inven_equ.add(39).writeU8(50);
                inven_equ.add(41).writeU8(50);
                inven_equ.add(42).writeU8(50);
                inven_equ.add(44).writeU8(50);
            ...
            // 执行原逻辑 todo 有问题
            Dispatcher_ResetRandomOption(a1, user, packet);
        }, 'int', ['pointer', 'pointer', 'pointer']
    ));
}
...
/**
 * 史诗魔法封印变更词条方法
 * @TODO 如果要使用的话，函数必须合并逻辑  魔法封印属性转换时可以继承  登录器 0627
 */
function hook_random_option_for_ss() {
    ...
    Interceptor.replace(Hook_Dispatcher_ResetRandomOption_process, new NativeCallback(function (a1, CUser, Msg) {
        ...
        if (CUser_CheckItemLock(CUser, 1, EquSlot) != 0) {
            ...
            var item = CDataManager_find_item(G_CDataManager(), equid);
            ...
            if (itemId == 15) {
                ...
                // Tips: "type":"error","description":"Error: access violation accessing 0x0","stack":"Error: access violation accessing 0x0\n
                //       arg0对应内存需要初始化，你用假紫去赛利亚那变换一次属性，再用fr的道具就可以用了
...
function change_new_material_required_for_shopping() {
    // PVF中[need material]词条
    Interceptor.attach(ptr(0x850d6f4), {
        // CItem::GetNeedMaterial
        onEnter: function (args) {
            var item = args[0]//目标 item = CDataManager_find_item(G_CDataManager(), item_id)
            var item_id = CItem_GetIndex(item)//目标id
        },
        onLeave: function (retval) {
            retval.add(0).writeU32(3262)//修改所需物品id
            retval.add(4).writeU32(13)//修改所需物品数量
        }
    });
}
...
function start() {
    ...
    // epicPotion() // 史诗药剂加成装备爆率
    // enable_drop_use_luck_piont(); // 使用角色幸运值加成装备爆率
    setMaxCAccountCargoSolt(120); // 账号金库扩展至120格
    disable_check_create_character_limit(); // 解除每日创建角色数量限制
    // DP_Strengthen_SendUpdateItemList(); // +13以上强化券自动刷新物品栏，暂时注释掉，可能带有镶嵌的装备，强化或增幅后，镶嵌信息会消失，需要重选角色或者整理背包才恢复
    ...
    set_equipment_unlock_time(4);	// 设置装备解锁时间，单位秒
