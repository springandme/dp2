...
-- 副职业一键分解券 分解装备前2栏 需要在开启个人分解机状态下使用
item_handler[2022110506] = function(user, item_id)
    item_id = nil;
    local q = 0
    for i = 9, 56, 1 do
        local info = dpx.item.info(user.cptr, game.ItemSpace.INVENTORY, i)
        if info then
            user:Disjoint(game.ItemSpace.INVENTORY, i, item_id)
            if not dpx.item.info(user.cptr, game.ItemSpace.INVENTORY, i) then
                q = q + 1
            end
        end
...
--        user:SendNotiPacketMessage("注意：品级相同才可以继承！")
    elseif math.abs(item2.usable_level - item1.usable_level) > 100 then
        user:SendNotiPacketMessage("注意：等级差必须小于等于100级的装备才可以被继承！")
    elseif item1.usable_level < 1 or item2.usable_level < 1   then
        user:SendNotiPacketMessage("注意：低于1级的装备不可以继承！")
